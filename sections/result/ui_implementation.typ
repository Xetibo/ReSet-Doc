...
There was one feature that was cut after some discussion which is the
...
#figure(
  img("reset_audio_profiles.png", fit: "contain", width: 85%), caption: [ReSet Audio profiles section],
)<reset_profiles>
...
As mentioned in @UserInterfaceGuidelines ReSet intends to be as consistent as possible,
which is why the same style principles were applied to all pages within ReSet.
...
#figure(
  sourcecode(
    ```rs
            ...
            ```,
  ), kind: "code", supplement: "Listing", caption: [ReSet sidebar entry],
)<d>
...
#figure(
  sourcecode(```rs
    ...
    ```), kind: "code", supplement: "Listing", caption: [Example GResources],
)<build_rs>
...
explained by an officially endorsed gtk-rs introduction @gtk_rs_introduction.
