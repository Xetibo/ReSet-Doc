...
a configuration file and can be modified using multiple ways.
...
read using kreadconfig6 and written to using kwriteconfig6. 
These are tools provided by KDE to modify settings @kdeconfig.

There was also a dbus interface that could be used to fetch keyboard 
...
An issue with these is that the versions are part of the command. 
...
that if version 7 is being released, the command needs to be 
...
Latin letters, Cyrillic letters etc. and make up a keyboard layout. It 
is not possible to just increase that number because the XKBState field
is only 16 bits long and uses bits 13 and 14 to report the keyboard group. 
...
while the rest have system colors. This number is set depending on the 
desktop environment because some don't use XKB and therefore could allow 
more than four keyboard layouts.
