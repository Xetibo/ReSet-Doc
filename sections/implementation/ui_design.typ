...
The initial UI mocks have been made in a generic PDF tool. The goal of these mocks is 
to get a general idea of how the UI should look like and to figure out a good user 
experience.
#align(center, [#figure(
...
#pagebreak()
There are multiple ways to build GTK UI. The first one is to use the GTK library itself to
build the UI directly in the code. The next two tools are UI builders like Glade @glade and 
Cambalache @cambalache. These tools will also be evaluated usingn a value table using a score 
range between 0 and 10 for each category. Each category is also given a weight to express impact.\
...
#grid(
  ...
  rows: (30pt),
  cell(
    [#figure([], kind: table, caption: [UI Builder tools])<programminglanguages>],
    bold: true,
  ),
)
...
#grid(
  ...
  cell("Glade", bold: true, use_under: true),
  cell("Cambalache", bold: true, use_under: true),
  cell("Weight", bold: true, use_under: true),
  cell("Functionality", bold: true, cell_align: left),
  cell([#gtk.functionality], bold: true),
  cell([#glade.functionality], bold: true),
  cell([#cambalache.functionality], bold: true),
  cell([\*#language_weights.functionality], bold: true),
  cell("Ease of Use", bold: true, fill: silver, cell_align: left),
  cell([#gtk.easeOfUse], fill: silver, bold: true),
  cell([#glade.easeOfUse], fill: silver, bold: true),
  cell([#cambalache.easeOfUse], fill: silver, bold: true),
  cell([\*#language_weights.easeOfUse], fill: silver, bold: true),
  cell("Collaboration", bold: true, cell_align: left),
  cell([#gtk.collaboration], bold: true),
  cell([#glade.collaboration], bold: true),
  cell([#cambalache.collaboration], bold: true),
  cell([\*#language_weights.collaboration], bold: true),
  cell("Debugging", bold: true, fill: silver, cell_align: left),
  cell([#gtk.debugging], fill: silver, bold: true),
  cell([#glade.debugging], fill: silver, bold: true),
  cell([#cambalache.debugging], fill: silver, bold: true),
  cell([\*#language_weights.debugging], fill: silver, bold: true),
  cell("Updates", bold: true, cell_align: left),
  cell([#gtk.updates], bold: true),
  cell([#glade.updates], bold: true),
  cell([#cambalache.updates], bold: true),
  cell([\*#language_weights.updates], bold: true),
  cell("Total", bold: true, fill: silver, cell_align: left),
  cell([#calculate_total(gtk, language_weights)], fill: silver, bold: true),
  cell([#calculate_total(glade, language_weights)], fill: silver, bold: true),
  cell([#calculate_total(cambalache, language_weights)], fill: silver, bold: true, color: green),
...
is a library wihch extends the features of GTK. It is developed by the GNOME project and thus 
follows the GNOME Human Interface Guidelines as well. It provides a lot of beautiful UI widgets 
that make the UI feel modern.

#text(12pt, [*UI structure*])\
The settings app can be divided into two parts. The left part is a sidebar which contains 
a list of all settings and the right side displays the actual setting where the user can 
make changes. While this is not a standard, it is a very common layout structure as seen in 
@Analysisofexistingapplications and even on other OS like Windows and macOS. The list of 
settings contains multiple categories like Connectivity and Sound, and each of them contains 
more subcategories.
