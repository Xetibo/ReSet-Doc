...
different strategies and reasons. For example, a compiler for languages such as
...
not work, as both functions have the same name, however, with mangling,
functions will have a seemingly random name and will therefore not clash with
each other.
...
Loading a shared library can either be done immediately, meaning all functions
of a library are loaded on startup, or the library functionality can be loaded
lazily. Lazy functions will only be loaded into memory when they are called.
This can be achieved with the global offset table.

The global offset table stores pointers to loaded libraries which will redirect
to the library functions. If the function is not yet loaded, then the global
offset will load the function into memory first.

In @global_offset_table an example function call with the global offset table is
...
Compared to an interpreted language, this is different from the fact that an
API-compatible change is not necessarily ABI-compatible. For example, changing a
...
Due to ABI instability and specific interpretation of memory, it is often
difficult if not impossible to provide interaction between languages if not used
on a stable ABI such as the C language. In this section, an example of C++ to
Rust compatibility is analyzed based on the Hyprland plugin system. @hyprland A
working proof of concept plugin without functionality can be examined in
...
```), kind: "code", supplement: "Listing", caption: [C++ struct])<cpp_struct>

Despite both Rust and C++ offering the same datatype, they are not properly
compatible. A single Rust String to C++ std::string is transferrable over a
shared library, however, a struct containing multiple strings is not converted
properly and results in a double free, meaning memory is attempted to be freed
twice by C++.
...
which any language can be used to interact with a shared library-based plugin
system. In order to achieve this goal, the application implementing the plugin
system needs to provide the entire API as a C API. This API can then be targeted
by other languages without knowing the implementation details. For proprietary
applications, this is specifically interesting, since they do not need to
provide any other code apart from the C header file.

In @hourglass and @hourglass_picture the architecture of the hourglass pattern
is visualized.
...
A potential shared library plugin system for ReSet could also implement this C
API in order to provide users of ReSet the possibility to use languages other
...
which will be evaluated in full before entering it into the macro. In other
words, the entire expression is multiplied by five, not just the first part of
it.

A different approach to Rust would be to offer a flag to use code at compile
time. This approach is used within C++ by adding constexpr or consteval to code.
