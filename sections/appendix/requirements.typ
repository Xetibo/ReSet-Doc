...
#requirement(
  "Seemless UI", "Performance", "High", [
    Settings should load quickly, so the user does not feel like the application is
    lagging
  ], [
...
#requirement(
  "User Interface", "Maintainability", "Medium", [
    In case of a plugin crash, the application should not crash. It should try to
    continue running or at least log the error.
  ], [
